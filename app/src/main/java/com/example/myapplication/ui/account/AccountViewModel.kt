...
class AccountViewModel : BaseViewModel() {
    ...
    var listGetUser = MutableLiveData<MutableList<User>>()
    ...
    fun getUser() {
        ...
        dbRoot.child(AppConst.LIST_USER_NODE).addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                listUser.clear()
                if (snapshot.exists()) {
                    for (item in snapshot.children) {
